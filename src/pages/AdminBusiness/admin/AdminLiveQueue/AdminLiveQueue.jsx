...
const BellIcon = () => (
  <svg width="14" height="14" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M10 17.5a1.667 1.667 0 0 0 1.667-1.667H8.333A1.667 1.667 0 0 0 10 17.5Z" fill="currentColor"/>
    <path d="M15.833 12.917V8.75a5.833 5.833 0 1 0-11.666 0v4.167L2.5 15v.833h15V15l-1.667-2.083Z" fill="currentColor"/>
  </svg>
);

const BackArrowIcon = () => (
  <svg width="14" height="14" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M17 10H3M3 10l5.5-5.5M3 10l5.5 5.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const CheckIcon = () => (
  <svg width="15" height="15" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M16.667 5 7.5 14.167 3.333 10" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const DeleteIcon = () => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M5.33333 5.33334V11.3333M8 5.33334V11.3333M10.6667 5.33334V11.3333M2.66667 3.99999H4.66667H13.3333M5.33333 3.99999V2.66666C5.33333 2.2984 5.63173 2 6 2H10C10.3683 2 10.6667 2.2984 10.6667 2.66666V3.99999M12.6667 3.99999V13.3333C12.6667 13.7016 12.3683 14 12 14H4C3.63173 14 3.33333 13.7016 3.33333 13.3333V3.99999H12.6667Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
...
const SummaryConnector = () => <span className="summary-arrow" aria-hidden="true" />;

const WAITING_PEOPLE = [
  {
    token: 'A129',
    name: 'John Kumar',
    waitingFor: 'Waiting for 12 min',
  ...
  },
  {
    token: 'A132',
    name: 'Mohammed Ali',
    waitingFor: 'Waiting for 3 min',
    phone: '+91 99887 76655',
    purpose: 'Lab Report Review',
    createdAt: '09:35 AM, 22 May 2024',
    estWait: '~19 min',
    source: 'Walk-in',
  },
];

const TokenDetailsPanel = ({ person, onClose }) => {
  if (!person) return null;
  const initial = person.name.trim().charAt(0).toUpperCase();

  return (
    <aside className="token-details-panel">
      <div className="token-details-body">
        <div className="token-details-header">
          <h2>Token Details</h2>
          <button type="button" className="token-details-close" aria-label="Close" onClick={onClose}>
            <CloseIcon />
          </button>
        </div>

        <div className="token-details-top">
          <p className="token-details-number">{person.token}</p>
          <span className="queue-badge waiting">Waiting</span>
        </div>

        <div className="token-details-person">
          <span className="token-details-avatar">{initial}</span>
          <div>
            <p className="token-details-name">{person.name}</p>
            <p className="token-details-phone">{person.phone}</p>
          </div>
        </div>

        <div className="token-details-purpose">
          <p className="token-details-label">Purpose of Visit</p>
          <p className="token-details-value">{person.purpose}</p>
        </div>

        <p className="token-details-section-title">Token Info</p>
      ...
      <div className="token-details-footer">
        <div className="token-details-footer-actions">
          <button type="button" className="small-button outline"><PauseIcon /> Hold</button>
          <button type="button" className="small-button outline plain"><BellIcon /> Notify</button>
        </div>
        <button type="button" className="token-details-delete"><DeleteIcon /> Delete Token</button>
      </div>
    </aside>
  );
};
    ...
    <div className={`page-admin-container ${isCollapsed ? 'collapsed' : 'expanded'}`}>
      <div className="page-inner-container">
        <div className="whistle-page-content">
          <section className="admin-live-queue-page">
            ...
            <div className="admin-queue-board">
              <div className="queue-column waiting-column">
                ...
                {WAITING_PEOPLE.map((person) => (
                  <div
                    key={person.token}
                    className={`queue-card list-card${selectedPerson?.token === person.token ? ' list-card--active' : ''}`}
                    role="button"
                    tabIndex={0}
                  ...
                  >
                    <div className="queue-card-heading">
                      ...
                      <p className="token-number">{person.token}</p>
                    </div>
                    <p className="token-name">{person.name}</p>
                    <p className="token-meta">{person.waitingFor}</p>
                  </div>
                ))}
                <button type="button" className="link-button orange">View All Waiting (86)</button>
              </div>

              <div className="queue-column violet-column">
                ...
                <div className="queue-card next-card">
                  <div className="spaced">
                    ...
                    <button type="button" className="icon-button" aria-label="Remove"><TrashIcon /></button>
                  </div>
                  <p className="token-name">Suresh Babu</p>
                  <p className="token-meta">+91 85468 32584</p>
                  <div className="card-actions">
                    <button type="button" className="small-button outline"><PauseIcon /> Hold</button>
                    <button type="button" className="small-button filled blue"><BellIcon /> Notify</button>
                  </div>
              ...
              <div className="queue-column blue-column">
                ...
                <div className="queue-card serving-card active-card">
                  ...
                  <div className="serve-status"><ClockIcon /> Started 4 min ago</div>
                  <div className="serving-actions">
                    <button type="button" className="small-button outline"><BackArrowIcon /> Back</button>
                    <button type="button" className="primary-outline-button"><CheckIcon /> Complete</button>
                  </div>
                </div>
              </div>
              ...
              <div className="queue-column green-column">
                ...
                <button type="button" className="link-button green">View All Completed</button>
              </div>
            </div>
          </section>
        </div>
      ...
      {selectedPerson && (
        <TokenDetailsPanel person={selectedPerson} onClose={() => setSelectedPerson(null)} />
      )}
    </div>
  );
}
